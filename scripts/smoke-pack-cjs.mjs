...
const smokeDir = join(rootDir, 'smoke/pack-node-cjs');
const tempDir = join(rootDir, '.tmp/pack-node-cjs');
...
// 2. Create tarball
execSync('pnpm pack --pack-destination ../../.tmp/pack-node-cjs', {
  cwd: packageDir,
  stdio: 'inherit',
});
...
console.log('Pack CJS smoke test passed');
